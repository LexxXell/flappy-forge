...
- User DB is JSON file persisted at `packages/web/data/users.json`.
...
- users DB: `packages/web/data/users.json`
...
- `Dockerfile` installs deps, builds web client once, then starts web server.
- `docker-compose.yml` mounts persistent host volumes for themes/builds/users DB.
